...
def render(plays: list, index, lane_b: dict[str, dict], reviewer: str = "") -> None:
    ...
    for s in _g(play, "steps", []) or []:
        ...
        with st.expander(head, expanded=dec is None):
            ...
            st.caption(f"Expected output — {expected}" if expected else "")
            reason = st.text_area(
                "Reason — which part of the expected output did you check, and what did you find?",
                key=f"rs_{key}", height=68,
                placeholder="e.g. register lists owner and risk tier for all 14 models, last updated 2026-08-30")
