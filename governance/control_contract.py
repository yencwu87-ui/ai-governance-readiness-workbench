...
Workbook authoring is normalized into per-framework YAML contracts. For MAS, requirement
semantics are overlaid from requirements/mas.yaml and remain authoritative. Workbook-derived
challenge/testing content is draft until formally approved.
"""
...
ALIASES = {
    'MAS':'MAS', 'ISO42001':'ISO 42001', 'ISO 42001':'ISO 42001',
    'NIST_AI_RMF':'NIST AI RMF', 'NIST AI RMF':'NIST AI RMF',
    'MGF_Agentic':'MGF Agentic', 'MGF Agentic':'MGF Agentic', 'SAFR':'SAFR'
}
...
def _framework_path(framework: str) -> Path:
    canonical = ALIASES.get(framework, framework)
    if canonical in FRAMEWORK_FILES:
        return CONTRACT_DIR / FRAMEWORK_FILES[canonical]
    return DEFAULT_PATH

def get_control_contract(control_id: str, framework: str = '') -> dict | None:
    p = _framework_path(framework)
...
def validate_per_framework() -> list[str]:
    errors=[]
    expected=set(FRAMEWORK_FILES.values())
    for fn in sorted(expected):
        p=CONTRACT_DIR/fn
        if not p.exists():
            errors.append(f'missing framework contract: {p}')
        else:
            errors.extend(validate_contracts(p))
    return errors
